...
MaterialPrincipled::MaterialPrincipled(Core *core, const glm::vec3 &base_color)
  ...
  core_->GraphicsCore()->CreateBuffer(sizeof(glm::vec3) * 4 + sizeof(float) * 15, graphics::BUFFER_TYPE_STATIC,
                                      &material_buffer_);
  sampler_implementation_ = {core_->GetShadersVFS(), "material/principled/sampler.hlsl"};
  evaluator_implementation_ = {core_->GetShadersVFS(), "material/principled/evaluator.hlsli"};
  power_sampler_implementation_ = {core_->GetShadersVFS(), "material/principled/power_sampler.hlsli"};
}
...
const CodeLines &MaterialPrincipled::PowerSamplerImpl() const {
