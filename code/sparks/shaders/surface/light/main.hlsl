...
[shader("callable")] void SurfaceMain(inout RenderContext context) {
  ...
  if (two_sided || context.hit_record.front_facing) {
    ...
    if (instance_meta.custom_index != -1) {
      ...
      uint primitive_index = context.hit_record.primitive_index;
      ...
      float pdf = (high_prob - low_prob) * context.hit_record.pdf;
      pdf *= DirectLightingProbability(instance_meta.custom_index);
      float3 omega_in = context.hit_record.position - context.origin;
      pdf *= dot(omega_in, omega_in);
      float NdotL = abs(dot(context.hit_record.geom_normal, normalize(omega_in)));
      pdf /= NdotL;
