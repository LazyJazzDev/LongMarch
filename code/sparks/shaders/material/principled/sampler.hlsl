...
#include "material/principled/principled_sheen.hlsli"
  ...
  void CalculateClosureWeight() {
    ...
    microfacet_closure.weight = float3(0, 0, 0);
    microfacet_closure.sample_weight = 0.0;
    microfacet_closure.N = float3(0, 0, 0);
    microfacet_closure.alpha_x = 0.0;
    microfacet_closure.alpha_y = 0.0;
    microfacet_closure.ior = 1.0;
    microfacet_closure.T = float3(0, 0, 0);
    microfacet_closure.color = float3(0, 0, 0);
    microfacet_closure.cspec0 = float3(0, 0, 0);
    microfacet_closure.fresnel_color = float3(0, 0, 0);

    microfacet_bsdf_reflect_closure.weight = float3(0, 0, 0);
    microfacet_bsdf_reflect_closure.sample_weight = 0.0;
    microfacet_bsdf_reflect_closure.N = float3(0, 0, 0);
    ...
    microfacet_bsdf_reflect_closure.T = float3(0, 0, 0);
    microfacet_bsdf_reflect_closure.color = float3(0, 0, 0);
    microfacet_bsdf_reflect_closure.cspec0 = float3(0, 0, 0);
    microfacet_bsdf_reflect_closure.fresnel_color = float3(0, 0, 0);

    microfacet_bsdf_refract_closure.weight = float3(0, 0, 0);
    microfacet_bsdf_refract_closure.sample_weight = 0.0;
    microfacet_bsdf_refract_closure.N = float3(0, 0, 0);
    microfacet_bsdf_refract_closure.alpha = 0.0;
    microfacet_bsdf_refract_closure.ior = 1.0;

    microfacet_clearcoat_closure.weight = float3(0, 0, 0);
    microfacet_clearcoat_closure.sample_weight = 0.0;
    microfacet_clearcoat_closure.N = float3(0, 0, 0);
    microfacet_clearcoat_closure.alpha = 0.0;
    microfacet_clearcoat_closure.ior = 1.0;
    microfacet_clearcoat_closure.cspec0 = float3(0, 0, 0);
    microfacet_clearcoat_closure.fresnel_color = float3(0, 0, 0);
    microfacet_clearcoat_closure.clearcoat = 0.0;

    sheen_closure.weight = float3(0, 0, 0);
    sheen_closure.sample_weight = 0.0;
    sheen_closure.N = float3(0, 0, 0);
    ...
    if (diffuse_weight > CLOSURE_WEIGHT_CUTOFF && sheen > CLOSURE_WEIGHT_CUTOFF) {
      ...
      float3 m_ctint = m_cdlum > 0.0f ? base_color / m_cdlum : float3(1, 1, 1);  // normalize lum. to isolate hue+sat

      float3 sheen_color = float3(1.0f - sheen_tint, 1.0f - sheen_tint, 1.0f - sheen_tint) + m_ctint * sheen_tint;
    ...
    if (specular_weight > CLOSURE_WEIGHT_CUTOFF &&
...

      {
        ...
        float3 m_ctint = m_cdlum > 0.0f ? base_color / m_cdlum : float3(1, 1, 1);  // normalize lum. to isolate hue+sat
        float3 tmp_col =
            float3(1.0f - specular_tint, 1.0f - specular_tint, 1.0f - specular_tint) + m_ctint * specular_tint;
    ...
    if (final_transmission > CLOSURE_WEIGHT_CUTOFF) {
      ...
      float3 cspec0 = base_color * specular_tint + 1.0f - specular_tint;
...

      {
        ...
        PREPARE_BSDF(microfacet_bsdf_refract_closure, base_color * glass_weight * refraction_fresnel);
        {
          ...
          microfacet_bsdf_refract_closure.alpha = saturatef(transmission_roughness * transmission_roughness);
    ...
    if (clearcoat > CLOSURE_WEIGHT_CUTOFF) {
...

      {
        ...
        microfacet_clearcoat_closure.cspec0 = float3(0.04f, 0.04f, 0.04f);
