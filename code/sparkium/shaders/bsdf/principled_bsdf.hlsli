...
SP_MUTATING void CalculateClosureWeight() {
  ...
  const float interface_ior = hit_record.front_facing ? ior : 1.0f / max(ior, 1e-6f);
  ...
  if (diffuse_weight > CLOSURE_WEIGHT_CUTOFF && sheen > CLOSURE_WEIGHT_CUTOFF) {
    ...
    float3 m_ctint = m_cdlum > 0.0f ? base_color / m_cdlum : make_float3(1);  // normalize lum. to isolate hue+sat
  ...
  if (specular_weight > CLOSURE_WEIGHT_CUTOFF &&
...

    {
      ...
      float3 m_ctint = m_cdlum > 0.0f ? base_color / m_cdlum : make_float3(1.0);  // normalize lum. to isolate hue+sat
...
EvalPrincipledBSDFKernel(in float3 omega_in, inout float pdf, in float3 eval, in float accum_weight, int exclude) {
  ...
  if (exclude != 1 && microfacet_closure.sample_weight >= CLOSURE_WEIGHT_CUTOFF) {
    eval +=
        bsdf_microfacet_ggx_eval_fresnel(microfacet_closure, omega_v, omega_in, local_pdf) * microfacet_closure.weight;
