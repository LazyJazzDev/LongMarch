...
PSOutput PSMain(PSInput input) {
  ...
  float3 albedo = SP_BINDING_material_data.Load<MaterialLambertian>(0).base_color;
  float3 emission = SP_BINDING_material_data.Load<MaterialLambertian>(0).emission;
  SP_BINDING_output.radiance = float4(emission, 0.0);
  SP_BINDING_output.albedo_roughness = float4(albedo, 1.0);
