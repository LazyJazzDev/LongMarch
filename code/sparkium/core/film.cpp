...
Film::Film(Core *core, int width, int height)
  ...
  core_->BackendDevice()->CreateImage(width, height, graphics::IMAGE_FORMAT_R32G32B32A32_SFLOAT, &raw_image_);
  core_->BackendDevice()->CreateImage(width, height, graphics::IMAGE_FORMAT_D32_SFLOAT, &depth_image_);
  core_->BackendDevice()->CreateImage(width, height, graphics::IMAGE_FORMAT_R32_SINT, &stencil_image_);
  core_->BackendDevice()->CreateBuffer(sizeof(int) + sizeof(float) * 3, graphics::BUFFER_TYPE_STATIC,
