...
SoftwarePipeline::SoftwarePipeline(Core *core, bool ray_query, ComputeShadingConfiguration shading)
    : core_(core),
...
#undef SAMPLE_SHADOW_NO_HITRECORD
  ...
  args.insert(args.end(), shading_.definitions.begin(), shading_.definitions.end());
  if (ray_query_)
    args.push_back("-DSPARKIUM_RAY_QUERY");
  if (has_graph)
    args.push_back("-DSPARKIUM_SHADER_GRAPHS");
  if (core_->GraphicsCore()->CreateShader(vfs, shading_.source, "Main", ray_query_ ? "cs_6_5" : "cs_6_0", args,
                                          &render_shader_))
  ...
  auto finalize = [&](graphics::ComputeProgram *program) {
    for (auto binding : std::vector<std::pair<graphics::ResourceType, uint32_t>>{
             {graphics::RESOURCE_TYPE_WRITABLE_IMAGE, shading_.output_images},
             {shading_.input_type, shading_.input_images},
             ...
             {graphics::RESOURCE_TYPE_STORAGE_BUFFER, buffers + 6u + shading_.extra_buffers},
  ...
  if (!shading_.auxiliary_entry.empty()) {
    if (core_->GraphicsCore()->CreateShader(vfs, shading_.source, shading_.auxiliary_entry, "cs_6_0", args,
                                            &auxiliary_shader_))
      throw std::runtime_error("failed to compile auxiliary compute shader");
    core_->GraphicsCore()->CreateComputeProgram(auxiliary_shader_.get(), &auxiliary_program_);
    finalize(auxiliary_program_.get());
  }
