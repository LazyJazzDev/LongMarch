...
// Kernel layout is supplied by the owning renderer. Shared registration and BVH
// construction do not select a realtime or path-tracing rendering policy.
struct ComputeShadingConfiguration {
  std::string source{"software/render.hlsl"};
  std::vector<std::string> definitions;
  uint32_t output_images{1};
  graphics::ResourceType input_type{graphics::RESOURCE_TYPE_WRITABLE_IMAGE};
  uint32_t input_images{1};
  uint32_t extra_buffers{};
  std::string auxiliary_entry;
};
...
class SoftwarePipeline {
 public:
  explicit SoftwarePipeline(Core *core, bool ray_query = false, ComputeShadingConfiguration shading = {});
  void ClearInstances();
  ...
  graphics::ComputeProgram *AuxiliaryProgram() const {
    return auxiliary_program_.get();
  }
 ...
 private:
  ...
  Core *core_;
  bool ray_query_;
  ComputeShadingConfiguration shading_;
  ...
  std::unique_ptr<graphics::Shader> render_shader_, auxiliary_shader_;
  std::unique_ptr<graphics::ComputeProgram> auxiliary_program_;
