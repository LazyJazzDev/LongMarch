#include "sparkium/pipelines/realtime/core/software_pipeline.h"
...
#undef SAMPLE_SHADOW_NO_HITRECORD
  ...
  auto finalize = [&](graphics::ComputeProgram *program) {
    for (auto binding : std::vector<std::pair<graphics::ResourceType, uint32_t>>{
             {graphics::RESOURCE_TYPE_WRITABLE_IMAGE, 3u},
             {graphics::RESOURCE_TYPE_IMAGE, 4u},
             {graphics::RESOURCE_TYPE_STORAGE_BUFFER, 1},
             {graphics::RESOURCE_TYPE_UNIFORM_BUFFER, 1},
             {graphics::RESOURCE_TYPE_STORAGE_BUFFER, buffers + 7u},
