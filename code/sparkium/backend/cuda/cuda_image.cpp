...
CudaImage::CudaImage(int width, int height, ImageFormat format)
  ...
  memory = std::make_unique<CudaMemory>(size_t(width) * height * external_bytes_);
}
...
void CudaImage::UploadData(const void *data, const Offset2D &o, const Extent2D &e) const {
...
void CudaImage::DownloadData(void *data, const Offset2D &o, const Extent2D &e) const {
  CheckRegion(extent_, o, e);
  std::vector<uint8_t> row(size_t(e.width) * external_bytes_);
  for (uint32_t y = 0; y < e.height; ++y) {
...
void CudaImage::Clear(const ClearValue &value) {
  if (unorm_) {
