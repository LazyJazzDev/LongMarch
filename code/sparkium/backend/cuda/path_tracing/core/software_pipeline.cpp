#include "sparkium/backend/cuda/path_tracing/core/software_pipeline.h"
...
SoftwarePipeline::SoftwarePipeline(Core *core, bool optix) : core_(core), optix_(optix) {
  if (optix_ && !core_->BackendDevice()->DeviceRayTracingSupport())
    throw std::runtime_error("OptiX traversal is unavailable");
}
...
void SoftwarePipeline::CompileRenderer(const std::vector<MaterialCode> &materials,
  ...
  if (core_->BackendDevice()->CreateShader(vfs, "software/render.hlsl", "Main", "cs_6_0", args, &render_shader_))
...
void SoftwarePipeline::Update(graphics::CommandContext *commands,
  ...
  if (HardwareTraversal()) {
    ...
    if (graphics::FrameProfile::active) {
      graphics::FrameProfile::active->counters["optix_hardware_traversal"] = 1;
