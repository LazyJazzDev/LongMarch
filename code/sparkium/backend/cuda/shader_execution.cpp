...
void CheckNVRTC(nvrtcResult result) {
  if (result != NVRTC_SUCCESS)
    throw std::runtime_error(std::string("compute NVRTC: ") + nvrtcGetErrorString(result));
}
...
void CudaShader::Impl::CompileCUDA(const std::string &code,
  ...
  if (optix_shader) {
    options.insert(options.end(), {"-DSLANG_CUDA_ENABLE_OPTIX", "-I" LONGMARCH_OPTIX_INCLUDE_DIR,
                                   "-I" SPARKIUM_CUDA_INCLUDE_DIR, "--relocatable-device-code=true"});
  }
#endif
...
#ifdef SPARKIUM_OPTIX_ENABLED
    optix_device = optix;
    optix_launch = std::make_unique<OptixLaunch>(optix, ptx, compute_entry, global_size);
#endif
  } else {
    CheckCUDA(cuModuleLoadData(&module, ptx.c_str()));
    CheckCUDA(cuModuleGetFunction(&kernel, module, compute_entry.c_str()));
...
void CudaShader::Impl::DispatchCUDA(const std::vector<uint8_t> &globals, uint32_t x, uint32_t y, uint32_t z) {
#ifdef SPARKIUM_OPTIX_ENABLED
  if (optix_launch) {
...
void CudaShader::Impl::ReleaseCUDA() {
  if (module)
