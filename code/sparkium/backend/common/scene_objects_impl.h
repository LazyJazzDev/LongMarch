...
SceneObjectSet<GeometryObject, TextureObject, MaterialObject, EntityObject>::SceneObjectSet(
    Core *core,
    std::shared_ptr<const SceneDefinition> source)
    : SceneObjects(std::move(source)) {
  if (!definition)
