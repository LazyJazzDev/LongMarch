...
#include "sparkium/backend/common/path_tracing/core/camera.h"
#include "sparkium/backend/common/path_tracing/core/film.h"
#include "sparkium/backend/common/path_tracing/core/scene.h"
...
bool Core::UsesGraphicsRayTracing() const {
  return BackendDevice()->DeviceRayTracingSupport() && BackendDevice()->API() != RenderBackend::CUDA;
}
...
void Core::LoadPublicShaders() {
  ...
  core_.BackendDevice()->CreateShader(vfs, "entity_chit.hlsl", "RenderClosestHit", "lib_6_5", {"-I."}, &shader);
  core_.SetPublicResource("mesh_lambertian_chit", std::move(shader));
  core_.BackendDevice()->CreateShader(vfs, "entity_chit.hlsl", "ShadowClosestHit", "lib_6_5", {"-I."}, &shader);
  ...
  core_.BackendDevice()->CreateShader(vfs, "entity_chit.hlsl", "ShadowAnyHit", "lib_6_5", {"-I."}, &shader);
