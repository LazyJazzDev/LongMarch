...
CpuImage::CpuImage(std::shared_ptr<const TextureData> texture)
    : extent_{uint32_t(texture->width), uint32_t(texture->height)},
      format_(IMAGE_FORMAT_R8G8B8A8_UNORM),
      channels_(4),
      external_bytes_(4),
      unorm_(true) {
  memory = std::make_unique<CpuMemory>(texture, texture->rgba.data(), texture->rgba.size());
}

CpuImage::CpuImage(int width, int height, ImageFormat format)
    : extent_{uint32_t(width), uint32_t(height)},
      format_(format),
      unorm_(false) {
  if (width <= 0 || height <= 0)
    throw std::invalid_argument("compute image dimensions must be positive");
  switch (format) {
    ...
    case IMAGE_FORMAT_R32_SINT:
      ...
      break;
    default:
      throw std::invalid_argument("unsupported compute image format");
  }
  external_bytes_ = channels_ * (unorm_ ? 1 : 4);
  if (uint64_t(width) * height > std::numeric_limits<size_t>::max() / (channels_ * 4))
    throw std::overflow_error("compute image size overflow");
  ...
  memory = std::make_unique<CpuMemory>(size_t(width) * height * external_bytes_);
}
...
void CheckRegion(Extent2D full, Offset2D o, Extent2D e) {
  if (o.x < 0 || o.y < 0 || uint64_t(o.x) + e.width > full.width || uint64_t(o.y) + e.height > full.height)
    throw std::out_of_range("compute image region");
}
}  // namespace

void CpuImage::UploadData(const void *data, const Offset2D &o, const Extent2D &e) const {
...
void CpuImage::DownloadData(void *data, const Offset2D &o, const Extent2D &e) const {
  CheckRegion(extent_, o, e);
  std::vector<uint8_t> row(size_t(e.width) * external_bytes_);
  for (uint32_t y = 0; y < e.height; ++y) {
...
void CpuImage::Clear(const ClearValue &value) {
  if (unorm_) {
