...
// DXC's single buffer-type templates predate Slang generics. Both compute targets
...
std::string LowerLegacySource(std::string s, const std::string &filename) {
  // Compute descriptor arrays are ordinary pointer-backed arrays, so this
  // graphics descriptor-indexing annotation has no compute semantic effect.
  ...
  if (filename == "buffer_helper.hlsli") {
    ...
    if (a == std::string::npos || b == std::string::npos || f == std::string::npos)
      throw std::runtime_error("buffer helper template layout changed; update compute lowering");
  ...
  if (s.find("template <") != std::string::npos)
    throw std::runtime_error("unsupported compute shader template in " + filename);
  Replace(s, ".template ", ".");
  ...
  // Keep register-space identity as a reflected attribute on compute targets.
  ...
  for (const char *type : {"float4", "float3", "float2", "float", "int", "uint"}) {
    Replace(s, std::string("RWTexture2D<") + type + ">", std::string("ComputeTexture_") + type);
    Replace(s, std::string("Texture2D<") + type + ">", std::string("ComputeTexture_") + type);
  }

  Replace(s, "SamplerState", "ComputeSamplerState");
  s = std::regex_replace(s, std::regex(R"(\bRayDesc\b)"), "ComputeRayDesc");
  // Use an explicit pointer/count descriptor span for resource arrays.
  s = std::regex_replace(
      s, std::regex(R"((RWByteAddressBuffer|ComputeTexture_\w+|ComputeSamplerState)\s+(\w+)\s*\[\s*\])"),
      "ComputeArray<$1> $2");
  return s;
}
...
std::string LowerLegacyHostSource(std::string text, const std::vector<std::pair<std::string, std::string>> &names) {
  ...
  const std::regex block(R"(\[ComputeBinding\([0-9]+\)\]\s*cbuffer\s+\w+\s*\{([^}]*)\}\s*;?)");
  std::smatch match;
  ...
  text = std::regex_replace(text, std::regex(R"(\[ComputeBinding\([0-9]+\)\])"), "__global export __extern_cpp");
