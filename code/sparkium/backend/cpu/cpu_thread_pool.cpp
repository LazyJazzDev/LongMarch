...
CpuThreadPool &CpuThreadPool::Shared() {
  static CpuThreadPool pool(ThreadCount(), Grain());
  return pool;
}

CpuThreadPool::CpuThreadPool(unsigned thread_count, uint64_t grain) : grain_(grain) {
  if (!grain)
...
void CpuThreadPool::Run(uint64_t count, const std::function<void(uint64_t, uint64_t)> &function) {
