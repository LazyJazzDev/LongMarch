...
void NativeShader::Impl::CompileCPU(const std::filesystem::path &directory,
  ...
  wrapper << Read(directory / "native_input.slang") << "\nexport __extern_cpp void " << native_entry << "_Run("
          ...
          << native_entry << "("
          << (explicit_context ? (call_arguments.empty() ? "native_context" : "native_context,") : "") << call_arguments
          << "); } } }\n";
  if (explicit_context) {
  ...
  Write(directory / "native_input.slang", wrapper.str());
  ...
  spAddTranslationUnitSourceFile(host_request, host_unit, (directory / "native_input.slang").string().c_str());
  ...
  host = reinterpret_cast<HostFunction>(library->findSymbolAddressByName((native_entry + "_Run").c_str()));
  if (!host)
    throw std::runtime_error("ordinary CPU function missing");
  if (explicit_context) {
    context_host = reinterpret_cast<ContextFunction>(host);
    auto abi = reinterpret_cast<uint64_t (*)()>(library->findSymbolAddressByName((native_entry + "_ABI").c_str()));
  ...
  for (auto &parameter : parameters) {
    if (explicit_context)
      continue;
    if (parameter.constants.empty()) {
      parameter.address = library->findSymbolAddressByName((native_entry + "_resource_" + parameter.name).c_str());
      if (!parameter.address)
        throw std::runtime_error("CPU resource symbol missing: " + parameter.name);
    } else {
      for (auto &field : parameter.constants) {
        field.address = library->findSymbolAddressByName((native_entry + "_constant_" + field.name).c_str());
        if (!field.address)
...
void NativeShader::Impl::DispatchCPU(const std::vector<uint8_t> &globals, uint32_t x, uint32_t y, uint32_t z) {
  ...
  if (explicit_context) {
    ...
    NativeCpuThreadPool::Shared().Run(groups,
  ...
  NativeCpuThreadPool::Shared().Run(groups, [&](uint64_t begin, uint64_t end) { host(begin, end, x, y); });
}
}  // namespace sparkium::backend
