...
template <typename Scalar>
LM_DEVICE_FUNC void SolveCubicPolynomial(Scalar a, Scalar b, Scalar c, Scalar d, Scalar *roots, int *num_roots) {
  Scalar x;
  Scalar y;
  Scalar deri;
  if (fabs(a) > 0) {
...
LM_DEVICE_FUNC bool EdgeEdgeCCD(const Vector3<Scalar> &p0,
  ...
  Scalar roots[3];
  ThirdOrderVolumetricPolynomial<Scalar>(p1 - p0, p2 - p0, p3 - p0, v1 - v0, v2 - v0, v3 - v0, polynomial_terms);
  int num_roots = 0;
  SolveCubicPolynomialLimitedRange(polynomial_terms[3], polynomial_terms[2], polynomial_terms[1], polynomial_terms[0],
                                   roots, &num_roots);
  ...
  for (int i = 0; i < num_roots; i++) {
    ...
    if (root >= 0) {
      if (EdgeEdgeIntersection<Scalar>(p0 + v0 * root, p1 + v1 * root, p2 + v2 * root, p3 + v3 * root)) {
        *t = root;
        return true;
      }
    }
...
LM_DEVICE_FUNC bool FacePointCCD(const Vector3<Scalar> &p0,
  ...
  Scalar polynomial_terms[4];
  Scalar roots[3];
  ThirdOrderVolumetricPolynomial<Scalar>(p0 - p, p1 - p, p2 - p, v0 - v, v1 - v, v2 - v, polynomial_terms);
  int num_roots = 0;
  SolveCubicPolynomialLimitedRange(polynomial_terms[3], polynomial_terms[2], polynomial_terms[1], polynomial_terms[0],
                                   roots, &num_roots);
  PrivateSort(roots, num_roots);
  for (int i = 0; i < num_roots; i++) {
    ...
    if (root >= 0) {
      if (FacePointIntersection<Scalar>(p0 + v0 * root, p1 + v1 * root, p2 + v2 * root, p + v * root)) {
        *t = root;
        return true;
      }
    }
