#include "xue_shan/draw/draw_core.h"
...
Core::Core(graphics::Core *core) : core_(core) {
  ...
  core_->CreateBuffer(1024, graphics::BUFFER_TYPE_DYNAMIC, &instance_index_buffer_);
...
void Core::Render(graphics::CommandContext *context, graphics::Image *image) {
  if (draw_metadata_.size() * sizeof(uint32_t) > instance_index_buffer_->Size()) {
    ...
    instance_index_buffer_->UploadData(instance_indices.data(), instance_indices.size() * sizeof(uint32_t));
  }
  ...
  metadata_buffer_->UploadData(draw_metadata_.data(), draw_metadata_.size() * sizeof(DrawMetadata));
  ...
  context->CmdBindResources(2, std::vector<graphics::Sampler *>{linear_sampler_.get()});
...
void Core::CmdSetDrawRegion(int x, int y, int width, int height) {
  commands_.push_back(std::make_unique<DrawCmdSetDrawRegion>(x, y, width, height));
  pixel_transform_ = PixelCoordToNDC(width, height);
}

void Core::CmdDrawInstance(Model *model, Texture *texture, const Transform &transform, glm::vec4 color) {
  CmdDrawInstance(model, texture->Image(), transform, color);
}

void Core::CmdDrawInstance(Model *model, const Transform &transform, glm::vec4 color) {
...
void Core::CmdDrawText(glm::vec2 origin, const std::string &text, glm::vec4 color) {
  ...
  for (size_t i = 0; i < wtext.size(); i++) {
    ...
    if (char_model.char_tex_) {
      ...
      CmdDrawInstance(text_model_.get(), char_model.char_tex_, pixel_transform_ * transform, color);
    }
...
void Core::CmdDrawInstance(Model *model, graphics::Image *texture, const Transform &transform, glm::vec4 color) {
  ...
  commands_.push_back(std::make_unique<DrawCmdDrawInstance>(model, texture, instance_base, instance_count));
}
