...
#ifdef LONGMARCH_OPTIX_ENABLED
  if (optix_) {
    output.construct<OptixAccelerationStructure>(optix_.get(), vertices, indices, vertex_count, stride,
                                                primitive_count, flags);
    return 0;
  }
#endif
  NativeUnsupported();
...
int NativeCore::CreateBottomLevelAccelerationStructure(Buffer *vertices,
  ...
  return CreateBottomLevelAccelerationStructure(vertices->Range(), indices->Range(), vertices->Size() / stride,
      stride, indices->Size() / 12, RAYTRACING_GEOMETRY_FLAG_NONE, output);
}
