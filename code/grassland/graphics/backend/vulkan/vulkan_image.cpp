...
void VulkanImage::UploadData(const void *data) const {
  ...
  core_->Device()->CreateBuffer(pixel_size * extent.width * extent.height,
                                VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                                VMA_MEMORY_USAGE_CPU_ONLY, &staging_buffer);
  std::memcpy(staging_buffer->Map(), data,
              pixel_size * extent.width * extent.height);
  staging_buffer->Unmap();
  vulkan::SingleTimeCommand(
      core_->TransferQueue(), core_->TransferCommandPool(),
      [&](VkCommandBuffer command_buffer) {
        vulkan::TransitImageLayout(
            command_buffer, image_->Handle(), VK_IMAGE_LAYOUT_GENERAL,
        ...
        vkCmdCopyBufferToImage(
            command_buffer, staging_buffer->Handle(), image_->Handle(),
            VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
        vulkan::TransitImageLayout(
            command_buffer, image_->Handle(),
