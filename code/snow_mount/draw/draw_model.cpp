...
Model::Model(Core *core) : core_(core) {
  core_->GraphicsCore()->CreateBuffer(
      sizeof(Vertex), graphics::BUFFER_TYPE_DYNAMIC, &vertex_buffer_);
  core_->GraphicsCore()->CreateBuffer(
      sizeof(uint32_t), graphics::BUFFER_TYPE_DYNAMIC, &index_buffer_);
}

void Model::SetModelData(const std::vector<Vertex> &vertices,
