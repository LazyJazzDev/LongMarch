...
PSOutput PSMain(PSInput input) {
  ...
  float3 normal = g_normal_texture.Load(int3(pixel_coord, 0)).xyz;
  output.exposure = float4(albedo.xyz * light_info.intensity * max(dot(normal, light_info.direction), 0.0), 0.0);
  return output;
}
