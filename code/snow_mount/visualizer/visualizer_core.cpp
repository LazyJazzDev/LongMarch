...
c #include "snow_mount/visualizer/visualizer_ownership_holder.h"
  ...
  int Core::Render(graphics::CommandContext * context, const std::shared_ptr<Scene> &scene,
    ...
    std::vector<graphics::Image *> images = {film->GetImage(FILM_CHANNEL_EXPOSURE), film->GetImage(FILM_CHANNEL_ALBEDO),
                                             film->GetImage(FILM_CHANNEL_POSITION),
                                             film->GetImage(FILM_CHANNEL_NORMAL)};
