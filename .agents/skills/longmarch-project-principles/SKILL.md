...
- Sparkium 的功能性 PR 必须图文搭配：正文说明功能变化、使用方式与验证结果，并配合能展示该功能的实际截图、渲染结果或前后对比图；架构与流程可辅以 Mermaid 示意图。
- 图片必须对应真实功能或测试结果，配有说明文字。渲染对比应标明场景、后端、分辨率、采样数及必要的比较条件，并说明验证范围；示意图不能冒充实际运行截图，低分辨率或低采样验证不能描述为全量验证。
- 正式 PR 报告的二进制插图按素材约定保存在 `assets/`，使用 Git LFS 和独立素材分支管理。PR 正文引用已发布、可访问的图片，优先固定到素材提交；注明相关素材 PR，不依赖审阅者无法访问的本机路径或 `out/` 临时文件。
- 提交或更新 PR 报告后，检查正文中的图片链接与展示内容，确保图片可访问、图注与数据一致，且报告描述的是最终实现。
...
- PR 合并前完成与改动范围相符的构建、测试和必要的人工检查，记录验证结果，并确认适用的 CI 与仓库合并要求已满足；内容性质的素材库 PR 按下文的自动合并例外处理。
...
- 仅添加或更新图片、模型、场景数据、报告插图及相关说明等内容的素材库 PR，无需额外检查、人工审阅或再次请求确认，发布后自动使用 squash merge 合并，再在主仓库引用实际合并后的素材提交。此规则明确授权该流程所需的素材分支推送、PR 创建与合并；仍不得直接推送主分支。
- 上述自动合并规则不适用于包含可执行代码、构建配置或工具逻辑修改的素材库 PR。主仓库更新素材引用时仍遵循其提交检查与 PR 流程，报告中的固定素材链接也应更新到已合并的提交。
- 大小写文件名冲突要区分命名、分别保存，并同步修改场景中的引用。不能用覆盖或丢弃其中一个文件来消除冲突。
- 修改素材时检查子模组指针与场景资源路径是否一致；不因普通素材更新改写旧 LongMarchAssets 仓库。

## 授权与维护

除上文明确授权的本地提交及内容性质素材库 PR 流程外，本 skill 不额外授予推送、合并或改写已发布历史的权限。当前任务中已有的明确授权继续有效，不重复请求确认；改写已发布历史时确认授权确实涵盖该改写范围，并用带旧提交校验的 `--force-with-lease`。
