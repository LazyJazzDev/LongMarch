...
- 当前正在开发的代码分支默认检出在仓库主工作目录，便于用户直接查看、编辑、构建和运行；“主工作目录”不等于 `main` 分支，开发仍在独立功能分支进行。
- 临时 worktree 可用于对比、验证或隔离操作，但不得长期将当前工作分支留在 `out/` 等临时目录。工作转为持续开发时，应将该分支迁回主工作目录，除非用户明确要求保留在其他目录。
- 迁回前检查主工作目录、临时 worktree 和子模块的修改，保留未提交文件及原有分支历史；先释放临时 worktree 对该分支的占用，再在主工作目录检出并同步子模块。保留仍被旧构建或测试记录引用的临时 worktree，构建目录按其实际源码路径继续区分，避免混用缓存。
...
- 在 macOS 上提交或更新图文报告时，所有应用界面截图统一使用 macOS 系统级窗口截图能力生成（例如系统截图工具或 `screencapture` 的窗口捕获），保留真实窗口外框、标题栏和系统阴影。不得以应用内部帧缓冲导出、无窗口截图或后期拼接的窗口与阴影替代；发布前检查截图确实包含窗口及阴影。
