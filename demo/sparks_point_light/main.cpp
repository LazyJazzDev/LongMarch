...
int main() {
  ...
  std::unique_ptr<XH::EntityPointLight> point_lights[num_lights];
  glm::vec3 positions[num_lights];

  for (int i = 0; i < num_lights; ++i) {
    point_lights[i] = std::make_unique<XH::EntityPointLight>(&sparks_core, glm::vec3{}, glm::vec3{1.0f}, 1.0f);
  ...
  while (!window->ShouldClose()) {
    ...
    window->SetTitle(std::string("Sparks Point Light - ") + fps_buf + "frames/s" + " - " + rps_buf + "Mrays/s");
  }
