...
int main() {
  ...
  sparkium::MaterialPrincipled material_light(&sparkium_core, {0.725, 0.71, 0.68});
  // sparkium::MaterialLambertian material_white(&sparkium_core, {0.8, 0.8, 0.8});
  // sparkium::MaterialLambertian material_red(&sparkium_core, {0.8, 0.0, 0.0});
  // sparkium::MaterialLambertian material_green(&sparkium_core, {0.0, 0.8, 0.0});
  // sparkium::MaterialPrincipled material_light(&sparkium_core, {0.8, 0.8, 0.8});
  material_light.emission_color = {1.0f, 1.0f, 1.0f};
  material_light.emission_strength = 30.0f;
  sparkium::MaterialPrincipled material_principled(&sparkium_core, {0.725, 0.71, 0.68});
  ...
  sparkium::EntityGeometryMaterial entity_tall_box(&sparkium_core, &geometry_tall_box, &material_principled);
  ...
  while (!window->ShouldClose()) {
    ...
    window->SetTitle(std::string("Sparkium Principled BSDF - ") + fps_buf + "frames/s" + " - " + rps_buf + "Mrays/s");
  }
