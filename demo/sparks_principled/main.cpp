...
int main() {
  ...
  XH::MaterialPrincipled material_light(&xh_core, {0.725, 0.71, 0.68});
  // XH::MaterialLambertian material_white(&xh_core, {0.8, 0.8, 0.8});
  // XH::MaterialLambertian material_red(&xh_core, {0.8, 0.0, 0.0});
  // XH::MaterialLambertian material_green(&xh_core, {0.0, 0.8, 0.0});
  // XH::MaterialPrincipled material_light(&xh_core, {0.8, 0.8, 0.8});
  material_light.emission_color = {1.0f, 1.0f, 1.0f};
  material_light.emission_strength = 30.0f;
  XH::MaterialPrincipled material_principled(&xh_core, {0.725, 0.71, 0.68});
  ...
  XH::EntityGeometryMaterial entity_tall_box(&xh_core, &geometry_tall_box, &material_principled);
