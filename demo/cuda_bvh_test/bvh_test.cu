...
LM_DEVICE_FUNC bool AnyHit(const Eigen::Vector3<float> &query, const SDFResult *result, const grassland::AABB &aabb) {
  ...
  for (int i = 0; i < 3; i++) {
    if (query[i] < aabb.lower[i]) {
      p[i] = aabb.lower[i] - query[i];
    } else if (query[i] > aabb.upper[i]) {
      p[i] = query[i] - aabb.upper[i];
    } else {
      p[i] = 0;
    }
  }
...
int main() {
  ...
  for (size_t i = 0; i < indices.size(); i += 3) {
    grassland::AABB aabb;
    aabb.lower = vertices[indices[i]];
    aabb.upper = vertices[indices[i]];
    for (size_t j = 1; j < 3; ++j) {
      aabb.lower = aabb.lower.cwiseMin(vertices[indices[i + j]]);
      aabb.upper = aabb.upper.cwiseMax(vertices[indices[i + j]]);
    }
