...
void Application::OnInit() {
  ...
  core_->CreateBuffer(vertices.size() * sizeof(glm::vec3), CD::graphics::BUFFER_TYPE_DYNAMIC, &vertex_buffer_);
  ...
  core_->CreateTopLevelAccelerationStructure(
      {blas_->MakeInstance(glm::mat4{1.0f}, 0, 0xFF, 0, CD::graphics::RAYTRACING_INSTANCE_FLAG_NONE)}, &tlas_);
  ...
  if (alive_) {
    ...
    tlas_->UpdateInstances(
        std::vector{blas_->MakeInstance(glm::rotate(glm::mat4{1.0f}, theta, glm::vec3{0.0f, 1.0f, 0.0f}), 0, 0xFF, 0,
                                        CD::graphics::RAYTRACING_INSTANCE_FLAG_NONE)});
  }
}
