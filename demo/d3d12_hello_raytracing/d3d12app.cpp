...
void Application::OnRender() {
  ...
  command_list_->Handle()->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

  D3D12_VERTEX_BUFFER_VIEW vertex_buffer_view = {};
  vertex_buffer_view.BufferLocation = vertex_buffer_->Handle()->GetGPUVirtualAddress();
  vertex_buffer_view.StrideInBytes = sizeof(Vertex);
  vertex_buffer_view.SizeInBytes = vertex_buffer_->Size();

  D3D12_INDEX_BUFFER_VIEW index_buffer_view = {};
  index_buffer_view.BufferLocation = index_buffer_->Handle()->GetGPUVirtualAddress();
...
void Application::CreatePipelineAssets() {
  ...
  device_->CreateBuffer(vertices.size() * sizeof(Vertex), D3D12_HEAP_TYPE_UPLOAD, &staging_vertex_buffer);
  device_->CreateBuffer(indices.size() * sizeof(uint32_t), D3D12_HEAP_TYPE_UPLOAD, &staging_index_buffer);

  device_->CreateBuffer(vertices.size() * sizeof(Vertex), D3D12_HEAP_TYPE_DEFAULT, &vertex_buffer_);
  device_->CreateBuffer(indices.size() * sizeof(uint32_t), D3D12_HEAP_TYPE_DEFAULT, &index_buffer_);
  std::memcpy(staging_vertex_buffer->Map(), vertices.data(), vertices.size() * sizeof(Vertex));
  ...
  device_->CreateShaderModule(CompileShader(GetShaderCode("shaders/main.hlsl"), "VSMain", "vs_6_0"), &vertex_shader_);
  device_->CreateShaderModule(CompileShader(GetShaderCode("shaders/main.hlsl"), "PSMain", "ps_6_0"), &pixel_shader_);
  ...
  pipeline_state_desc.InputLayout = {input_element_descs.data(), UINT(input_element_descs.size())};
