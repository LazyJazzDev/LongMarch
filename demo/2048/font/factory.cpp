...
int directional_scan(const glm::ivec2 &v, const glm::ivec2 &u, const glm::ivec2 &p) {
  auto area = cross(v, u);
  auto rv = rotate90(v);
  auto ru = rotate90(u);
...
void merge_contours(std::vector<glm::ivec2> &outer_contour,
  ...
  for (int i = 0; i < outer_contour.size(); i++) {
    ...
    for (int j = 0; j < inner_contour.size(); j++) {
      if (std::abs(signal(cross(outer_contour[i] - outer_contour[i_1], inner_contour[j] - outer_contour[i_1])) -
                   signal(outer_area)) == 2)
        continue;
      if (std::abs(signal(cross(outer_contour[i] - inner_contour[j], outer_contour[i1] - inner_contour[j])) -
                   signal(outer_area)) == 2)
        continue;
...
void triangulation(std::vector<glm::vec2> &triangles, std::vector<glm::ivec2> &outline) {
  int area = 0;
  ...
  if (!direction) {
    grassland::LogError("Outline has no area.");
  }
  ...
  while (outline.size() > 2) {
    ...
    while (mid_index < sz) {
      ...
      triangle = Triangle{outline[prev_index], outline[mid_index], outline[next_index]};

      auto triangle_direction =
          signal(cross(outline[mid_index] - outline[prev_index], outline[next_index] - outline[prev_index]));
      ...
      if (pass) {
        auto this_area = std::abs(cross(triangle.v1 - triangle.v0, triangle.v2 - triangle.v0));
    ...
    triangle = Triangle{outline[prev_index], outline[mid_index], outline[next_index]};
    if (result_area > 0) {
      triangles.emplace_back(float(triangle.v0.x) * inv_size, float(triangle.v0.y) * inv_size);
      triangles.emplace_back(float(triangle.v1.x) * inv_size, float(triangle.v1.y) * inv_size);
      triangles.emplace_back(float(triangle.v2.x) * inv_size, float(triangle.v2.y) * inv_size);
    }
...
void CheckFreeTypeError(FT_Error error, const char *expression) {
  if (error) {
    throw std::runtime_error(fmt::format("FreeType error {} in {}", int(error), expression));
  }
}

#define CheckFreeType(x) CheckFreeTypeError((x), #x)
}  // namespace
...
Mesh::Mesh(const std::vector<glm::vec2> &vertices, const std::vector<uint32_t> &indices, float advance) {
...
Factory::Factory(const std::string &font_file_path) {
  CheckFreeType(FT_Init_FreeType(&library_));
  CheckFreeType(FT_New_Face(library_, font_file_path.c_str(), 0, &face_));
  CheckFreeType(FT_Set_Pixel_Sizes(face_, 0, size_scale));
}
...
void Factory::LoadChar(Char_T c) {
  CheckFreeType(FT_Load_Char(face_, c, FT_LOAD_DEFAULT));
  ...
  outline_funcs.move_to = [](const FT_Vector *to, void *user) -> int {
  ...
  outline_funcs.line_to = [](const FT_Vector *to, void *user) -> int {
    auto *outlines = static_cast<Outlines *>(user);
  ...
  outline_funcs.conic_to = [](const FT_Vector *control, const FT_Vector *to, void *user) -> int {
    auto *outlines = static_cast<Outlines *>(user);
    ...
    for (int i = 1; i < num_frac; i++) {
      ...
      outlines->buffer.push_back((outlines->pos * comp_i * comp_i + 2 * vec_control * i * comp_i + vec_to * i * i) >>
                                 (2 * arc_precision));
    }
  ...
  outline_funcs.cubic_to = [](const FT_Vector *control1, const FT_Vector *control2, const FT_Vector *to,
                              void *user) -> int {
    auto *outlines = static_cast<Outlines *>(user);
    ...
    for (int i = 1; i < num_frac; i++) {
      auto comp_i = num_frac - i;
      outlines->buffer.push_back((outlines->pos * comp_i * comp_i * comp_i + vec_control1 * comp_i * comp_i * i * 3 +
                                  vec_control2 * comp_i * i * i * 3 + vec_to * i * i * i) >>
                                 (3 * arc_precision));
    }
    outlines->buffer.push_back(vec_to);
    outlines->pos = vec_to;
  ...
  CheckFreeType(FT_Outline_Decompose(&outline, &outline_funcs, &outlines));
  if (!outlines.buffer.empty()) {
    outlines.outlines.push_back(outlines.buffer);
    outlines.buffer.clear();
  }
  ...
  while (!process_set.empty()) {
    for (auto i : process_set) {
      ...
      if (father == -1) {
        ...
        merge_contours(outlines.outlines[father], outlines.outlines[i], outlines.outlines);
        alive_set.erase(i);
  ...
  loaded_fonts_.insert(std::make_pair(c, Mesh(vertices, float(face_->glyph->advance.x) * inv_size)));
}
