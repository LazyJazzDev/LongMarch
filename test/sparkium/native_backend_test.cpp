...
TEST_P(NativeBackendTest, ThreeDimensionalInvocationIdsAndConstantBlock) {
...
TEST_P(NativeBackendTest, FiniteLightClassification) {
...
TEST_P(NativeBackendTest, MeshMaterialAndBVHWithAnalyticEmission) {
  sparkium::Core renderer(core.get());
...
TEST_P(NativeBackendTest, ExplicitContextRebindingAndCachedModules) {
  sparkium::Core renderer(core.get());
  auto vfs = renderer.GetShadersVFS();
...
#define PARAMS NativeResource<ConstantBuffer<Settings>>(native_context,0)
#define OUTPUT NativeResource<RWByteAddressBuffer>(native_context,1)
#else
...
#include "native_contract.hlsli"
#ifdef SPARKIUM_CPU_FUNCTIONS
#define SP_BINDING_data_buffers NativeResource<NativeArray<RWByteAddressBuffer>>(native_context,0)
#define SP_BINDING_software_nodes NativeResource<RWByteAddressBuffer>(native_context,1)
#define SP_BINDING_software_instances NativeResource<RWByteAddressBuffer>(native_context,2)
#define RAYS NativeResource<RWByteAddressBuffer>(native_context,3)
#define OUTPUT NativeResource<RWByteAddressBuffer>(native_context,4)
#else
