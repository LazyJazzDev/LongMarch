...
TEST(CudaRayTracing, HardwareHitsTransformsMasksAndEmptyScene) {
  ...
  ASSERT_EQ(core->CreateBottomLevelAccelerationStructure(vertices->Range(), triangles->Range(), 3, 12, 1,
      graphics::RAYTRACING_GEOMETRY_FLAG_NONE, &blas), 0);
  EXPECT_THROW(core->CreateBottomLevelAccelerationStructure(vertices->Range(), triangles->Range(), 3, 12, 2,
      graphics::RAYTRACING_GEOMETRY_FLAG_NONE, &tlas), std::out_of_range);
  ...
  std::vector<graphics::RayTracingInstance> instances{
      blas->MakeInstance(glm::mat4x3(1), 17, 1),
...
[shader("raygeneration")] void Main() {
  ...
  auto as_float = [](uint32_t bits) { float value; std::memcpy(&value, &bits, 4); return value; };
  auto hit = trace(1, 1);
  EXPECT_EQ(hit[0], 17); EXPECT_EQ(hit[1], 0);
  EXPECT_FLOAT_EQ(as_float(hit[2]), 2);
  EXPECT_FLOAT_EQ(as_float(hit[3]), 0.25f); EXPECT_FLOAT_EQ(as_float(hit[4]), 0.5f);
  EXPECT_EQ(hit[5], 31); EXPECT_FLOAT_EQ(as_float(hit[7]), 1);
  EXPECT_EQ(hit[10], UINT32_MAX); EXPECT_EQ(hit[15], UINT32_MAX);
  hit = trace(1, 3);
  EXPECT_EQ(hit[0], 18); EXPECT_FLOAT_EQ(as_float(hit[2]), 1.5f);
  instances[1] = blas->MakeInstance(transform(6, 1), 31, 1);
  ASSERT_EQ(tlas->UpdateInstances(instances), 0);
  hit = trace(1, 1);
  EXPECT_EQ(hit[5], UINT32_MAX); EXPECT_EQ(hit[10], 31);
  ASSERT_EQ(tlas->UpdateInstances(std::vector<graphics::RayTracingInstance>{}), 0);
  hit = trace(0, 1);
  for (int i = 0; i < 4; ++i) EXPECT_EQ(hit[i * 5], UINT32_MAX);
  ASSERT_EQ(tlas->UpdateInstances(instances), 0);
  hit = trace(1, 1);
  EXPECT_EQ(hit[0], 17); EXPECT_EQ(hit[10], 31);
}
