...
Build `demo_sparkium_gui` and run:

```sh
build-release/demo/sparkium_gui/demo_sparkium_gui assets/scenes --backend cuda --display-backend vulkan
```

The render worker owns CUDA and OptiX. A separate graphics device owns the GUI
window and displays completed RGBA frames. The Render backend dropdown retains
live CPU/CUDA/graphics switching; CUDA Auto chooses OptiX when available.
Choose Path Tracing for explicit hardware tracing or Path Tracing - Fallback
for CUDA software BVH traversal. Pipeline changes reset accumulation.
CPU continues to use embedded LLVM function JIT, the persistent `std::thread`
pool and its own SAH BVH. Slang is supplied by the existing vcpkg package.
