...
- There is no shared backend implementation directory. Each backend owns its
  material code generator, shader graph compiler and trace data layout.
- CPU and CUDA have separate buffer/image/memory/sampler/program/command-context
  types, binding descriptors, Slang sessions and shader frontends. Resource
  constructors do not accept a runtime CPU/CUDA switch. CPU memory can retain
  immutable scene data; CUDA memory exclusively owns device allocations.
- CPU compilation reflects invocation semantics, then JIT-compiles ordinary LLVM
  functions for its thread pool. CUDA emits CUDA source and compiles PTX through
  NVRTC; only that backend knows about OptiX resource binding and launch state.
...
CPU/CUDA `rasterization`, inline `ray_query`, presentation and CUDA/graphics
...
The optional CPU/CUDA backends requires the Slang compiler **library and headers**.
...
cmake --build build --target demo_sparkium_cli sparkium_compute_test \
  sparkium_fallback_test -j6
```

`LONGMARCH_ENABLE_COMPUTE_RENDER` defaults to `ON`. Existing build scripts must
use this renamed option; no alias for the former option is retained. Diagnostic
environment variables use the `SPARKIUM_COMPUTE_` prefix, and test executables
use `sparkium_compute_`. CMake discovers the vcpkg
...
If either target is absent, configuration reports CPU/CUDA rendering disabled;
the existing graphics backends remain buildable. Set
`-DLONGMARCH_ENABLE_COMPUTE_RENDER=OFF` to disable it explicitly.
...
cmake --build build-cpu --target demo_sparkium_cli sparkium_compute_test -j6
```
...
```text
Shared HLSL + explicit compute contract -> Slang IR -> LLVM IR -> in-process JIT -> CPU function
```
...
`sparkium/backend/{graphics,cpu,cuda}` implements Sparkium's rendering devices and
...
Only three parallel prefix-scan shaders need `SPARKIUM_CPU` variants:
...
Sparkium shaders use `compute_contract.hlsli`: explicit macros describe buffer
...
Each CPU call receives a `ComputeContext*` followed by the workgroup range and grid
...
differ from the CPU/CUDA float implementation.
...
All pass the unchanged 0.01 CPU/CUDA comparison threshold. These checks establish
...
The CPU/CUDA path tracers retains the baseline's:

| Feature family | CPU/CUDA path |
| --- | --- |
...
traversal adapter. The CPU/CUDA backends does not add volumes, denoising, motion
...
```sh
build/test/sparkium/sparkium_compute_test
python3 scripts/check_compute_backends.py \
  --cli build/demo/sparkium_cli/demo_sparkium_cli \
  --output out/compute-validation --repeat --no-gpu-cpu
```
...
The C++ compute-backend tests cover descriptor-array ABI, buffer subranges, retained
...
- `SPARKIUM_COMPUTE_TRACE=1`: reflected bindings and compute dispatches.
- `SPARKIUM_COMPUTE_DUMP=/path`: Slang intermediates, complete adapted CPU sources
  and generated CUDA source.
- The previous `SPARKIUM_COMPUTE_ASAN` option is rejected: downstream C++ ASan
  instrumentation is not available in the direct LLVM JIT pipeline.
- CUDA `compute-sanitizer --tool memcheck` and Nsight Systems work on the CUDA
